-- Launch audit: webhook idempotency, session/set indexes, AI commit skips completed sessions.
...
comment on table public.revenuecat_webhook_events is
  'Idempotency log for RevenueCat webhook deliveries. Service-role only.';

alter table public.revenuecat_webhook_events enable row level security;

revoke all on table public.revenuecat_webhook_events from public;
revoke all on table public.revenuecat_webhook_events from anon;
revoke all on table public.revenuecat_webhook_events from authenticated;
grant all on table public.revenuecat_webhook_events to service_role;

create index if not exists idx_v2_workout_sessions_user_status_completed
  on public.v2_workout_sessions (user_id, status, completed_at desc);

create index if not exists idx_v2_session_sets_performed_at
  on public.v2_session_sets (performed_at)
