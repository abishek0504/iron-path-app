-- Exclude stretch exercises from PR detection (mobility holds are not performance PRs).
...
BEGIN
  ...
  IF v_exercise_id IS NOT NULL THEN
    SELECT e.is_stretch INTO v_is_stretch
    FROM v2_exercises e
...
COMMENT ON FUNCTION trigger_upsert_exercise_pr() IS
  'Upserts v2_user_exercise_prs: weight, reps_only, timed. Warm-up sets and stretch exercises (is_stretch) are ignored.';
