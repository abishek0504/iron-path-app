...
63. **20260903140000_ai_replace_plan_day_on_commit.sql** - Adds `clear_plan_day_for_ai_replace(user, day, session_start, session_end)` (delete that day’s `v2_template_slots`, unperformed `v2_session_sets` / `v2_session_exercises` in the date window, then empty active sessions). Rewrites `commit_ai_generation` to call the clearer **before** insert and skip duplicate `exercise_id`s in one job. EXECUTE on the helper is revoked from public/anon/authenticated; `commit_ai_generation` remains service_role only. Live also applied the helper, the commit rewrite, and the empty-session delete as `ai_replace_plan_day_on_commit`, `ai_replace_day_in_commit_ai_generation`, and `clear_plan_day_delete_empty_sessions`.
64. **20260903150000_resolve_ai_targets_fill_null_weight.sql** - `resolve_ai_exercise_targets` still prefers the LLM’s sets/reps/duration/weight. When the LLM returns valid sets+reps but `weight` is null, it fills a starting load from the active experience prescription (`suggested_weight_multiplier_bw * bodyweight`, else suggested lbs/kg). Timed/stretch stays null weight. EXECUTE remains service_role only. Applied live as `resolve_ai_targets_fill_null_weight`.
