...
- ✅ AI generate still loads 48h muscle freshness and 60-day completed-set history into the prompt. `resolve_ai_exercise_targets` fills prescription × bodyweight when the LLM leaves `weight` null; client fallback uses `selectExerciseTargets` (history first). Performed sets, PRs, and completed sessions survive replace. Planner add / reorder / remove / edit are unchanged.
