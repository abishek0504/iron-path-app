...
5. Validate in `finalizeAiSessions`: every returned ID must be in the catalog (allow-list is the security boundary); drop off-focus and avoided-muscle strength lifts; dedupe by `exercise_id` **and** normalized name across the day (one Hip Thrust identity, not seven copies); per-session strength count must match the requested count or the auto min/max (else the whole result is rejected → fallback); pad missing strength from the **focused** catalog; out-of-bounds or mode-mismatched targets are nulled (client / `resolve_ai_exercise_targets` falls back to prescription-based targets) rather than failing generation. If the LLM returns valid sets+reps but `weight` is null (no 60-day history, or it cannot estimate safely), `resolve_ai_exercise_targets` keeps those sets/reps and fills weight from `v2_exercise_prescriptions` using `current_weight * suggested_weight_multiplier_bw` (else suggested lbs/kg). The client fallback in `executeAiDayGeneration` does the same via `selectExerciseTargets` so progressive-overload history is used when present. Bodyweight / timed / stretch lifts stay at null weight.
